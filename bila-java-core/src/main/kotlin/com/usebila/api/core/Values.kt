...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { BilaInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw BilaInvalidDataException("`$name` is not set")
            is JsonNull -> throw BilaInvalidDataException("`$name` is null")
            else -> throw BilaInvalidDataException("`$name` is invalid, received $this")
        }
