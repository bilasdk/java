...
[![Maven Central](https://img.shields.io/maven-central/v/com.usebila.api/bila-java)](https://central.sonatype.com/artifact/com.usebila.api/bila-java/0.1.0)
[![javadoc](https://javadoc.io/badge2/com.usebila.api/bila-java/0.1.0/javadoc.svg)](https://javadoc.io/doc/com.usebila.api/bila-java/0.1.0)
...
Javadocs are available on [javadoc.io](https://javadoc.io/doc/com.usebila.api/bila-java/0.1.0).
...
```kotlin
implementation("com.usebila.api:bila-java:0.1.0")
```
...
| --------- | --------------- | -------------------- | -------- | --------------------------- |
| `apiKey`  | `bila.apiKey`   | `BILA_API_KEY`       | true     | -                           |
| `baseUrl` | `bila.baseUrl`  | `BILA_BASE_URL`      | true     | `"https://api.usebila.com"` |
...
To send a request to the Bila API, build an instance of some `Params` class and pass it to the corresponding client method. When the response is received, it will be deserialized into an instance of a Java class.

For example, `client.accounts().list(...)` should be called with an instance of `AccountListParams`, and it will return an instance of `AccountListResponse`.
...
CompletableFuture<AccountListResponse> accounts = client.async().accounts().list();
```
...
// Configures using the `bila.apiKey` and `bila.baseUrl` system properties
// Or configures using the `BILA_API_KEY` and `BILA_BASE_URL` environment variables
BilaClientAsync client = BilaOkHttpClientAsync.fromEnv();

CompletableFuture<AccountListResponse> accounts = client.accounts().list();
```
...
int statusCode = accounts.statusCode();
Headers headers = accounts.headers();
```
...
Enable logging by setting the `BILA_LOG` environment variable to `info`:

```sh
export BILA_LOG=info
```

Or to `debug` for more verbose logging:

```sh
export BILA_LOG=debug
```
...
Although the SDK uses reflection, it is still usable with [ProGuard](https://github.com/Guardsquare/proguard) and [R8](https://developer.android.com/topic/performance/app-optimization/enable-app-optimization) because `bila-java-core` is published with a [configuration file](bila-java-core/src/main/resources/META-INF/proguard/bila-java-core.pro) containing [keep rules](https://www.guardsquare.com/manual/configuration/usage).
...
We are keen for your feedback; please open an [issue](https://www.github.com/bilasdk/java/issues) with questions, bugs, or suggestions.
